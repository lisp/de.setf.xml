...
;;;
;;; 2005-11-28  janderson  added ignore whitespace arg with default nil
;;; 2005-12-12  janderson  changed debugging output
...
(defun xml-rpc-decode-type (argument-expression)
  (destructure-element ((gi) encoded-value) argument-expression
    (validate-gi gi '{}value)
    (if (stringp encoded-value)
      'string
...
(defstruct (xml-rpc-struct
            (:constructor %make-xml-rpc-struct))
  members
  )

(defun make-xml-rpc-struct (&rest member-specs)
  (declare (dynamic-extent member-specs))
  (%make-xml-rpc-struct
   :members (loop for ((field-name field-type) field-value) on member-specs by #'cddr
                  collect (make-xml-rpc-member
                           :name field-name
                           :type field-type
                           :value field-value))))

(defstruct xml-rpc-member name type value encoding)

(defun xml-rpc-slot-value (struct name)
  (dolist (member (xml-rpc-struct-members struct))
    (when (string-equal name (xml-rpc-member-name member))
      (return (xml-rpc-member-value member)))))

(defun xml-rpc-slot-type (struct name)
  (dolist (member (xml-rpc-struct-members struct))
    (when (string-equal name (xml-rpc-member-name member))
      (return (xml-rpc-member-type member)))))
;;; (make-xml-rpc-struct '("m1" :string) "v1" '("m2" :integer) 2)

(defGeneric xml-rpc-decode-value-string (type-name content)
  (:method ((type-name (eql 'integer)) content)
           (parse-integer (trim-string-whitespace (first content))))
  (:method ((type-name (eql 'double)) content)
           (parse-double (trim-string-whitespace (first content))))
  (:method ((type-name (eql 'string)) content)
           "if the element has no content, return a zero-length string"
           (let ((value (first content)))
                (or value "")))
  ...
  (:method ((type-name (eql 'cons)) content)
           (let ((struct (make-xml-rpc-struct)))
             (setf (xml-rpc-struct-members struct)
                   (mapcar #'(lambda (child)
                               (destructure-element ((gi) name value) child
                                 ...
                                 (make-xml-rpc-member
                                  :name (make-uname (first (children name))
                                                    :namespace "")
                                  :value (xml-rpc-decode-value value)
                                  :type (xml-rpc-decode-type value))))
                           content))
             struct))
...
(defun serve-method-call (uri input-source output-destination
                          &key ((:xml-rpc-package *xml-rpc-package*)
                               ...
                               (namespace-bindings
                                `(("" . ,*xml-rpc-package*))))
  (handler-case
    (let* ((document (document-parser input-source :ignore-whitespace ignore-whitespace))
           (result nil))
...
<p>
the server method definition is a simple defmethod form</p>
...
(defMethod |system.methodSignature| ((name string))
  (let ((function (intern-function name)))
    (if (typep function 'generic-function)
      (let* ((method (first (closer-mop:generic-function-methods function)))
...
(define-symbol-macro *call-served-method.verbose*
  (get 'call-served-method :verbose))
;; (setf de.setf.xml.xml-rpc::*call-served-method.verbose* t) nil) t)


(defparameter *call-served-method.ignore-whitespace* nil)

(defun call-served-method (uri method-name &rest arguments)
  (let* ((request-content
          (with-output-to-string (content-stream)
            (with-xml-writer (content-stream :pretty nil)
              (xml {}methodCall
                   (xml {}methodName (encode-string method-name))
                   (xml {}params
                        (dolist (argument arguments)
                          (xml {}param
                               (xml {}value (xml-rpc-encode-value argument)))))))))
         (request-length (length request-content))
         (ignore-whitespace *call-served-method.ignore-whitespace*))
    (when *call-served-method.verbose*
      (format *trace-output* "~&call-served-method: request: (~s) ~s ~s ~s"
              request-length uri method-name request-content))
    (with-http-stream (http-stream uri
      ...
      (let ((buffer (make-array 1024 :fill-pointer 0
            ...
            (byte 0))
        ;; this should skip the buffer and parse from the stream, (especially to
        ;; handle decoding, but the buffer is nice to demonstrate and examine results
        (loop (unless (setf byte (read-byte http-stream nil nil)) (return))
          (vector-push-extend (code-char byte) buffer))
        (when *call-served-method.verbose*
          (format *trace-output* "~&call-served-method: response buffer: ~s" buffer))
        (let* ((result (document-parser buffer :ignore-whitespace ignore-whitespace)))
          (when *call-served-method.verbose*
            (format *trace-output* "~&call-served-method: response document: ~s" result))
          (values
           (destructure-element ((gi) response-parameters) (root result)
                                (validate-gi gi '{}methodResponse)
             ;; (print (list gi response-parameters))
             (destructure-element ((gi) response-value) response-parameters
               ;; (print (list gi response-value))
                     ...
                     ((eq gi '{}fault)
                      (let ((error-values (xml-rpc-decode-value response-value)))
                        (error "server-fault: ~s: ~a"
                               (xml-rpc-slot-value error-values '{}faultCode)
                               (xml-rpc-slot-value error-values '{}faultString)))))))
           result
           buffer))))))
...
;;; eliminated an example as it required an inline package definition
