...
<DOCUMENTATION>
 ...
 <CHRONOLOGY>
  ...
  <DELTA DATE='20020702'>corrected for name instances, but untested</DELTA>
  </CHRONOLOGY>
 </DOCUMENTATION>
|#
...
(defGeneric write-node-graph (node stream)
  ...
  (:method (node (*standard-output* stream) &aux (*print-pretty* t))
  ...
  (flet ((format-node (name properties)
    ...
    (cond ((setf cache-entry (gethash node *node-graph-cache*))
          ...
          (t ;generate new node and recurse
           (setf id (gensym (concatenate 'string (local-part (name node)) "-"))
...
(defMethod encode-node-graph ((node elem-property-node) &aux name id)
  (setf name (name node))
  (when (eq name *default-prefix-name*)
    (setf name "xmlns"))
  (setf id (gensym (concatenate 'string (local-part name) "-")))
...
(defMethod encode-node-graph ((node def-elem-type) &aux cache-entry id props c-def)
  (flet ((format-node (name properties)
           (format *standard-output* "~a  \"~a\" ~@[[~{~a=\"~a\"~^,~}]~];"
                   *eol* name properties))
         (format-link (from to properties)
